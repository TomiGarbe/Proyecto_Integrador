...
@router.put("/update-user/{id_user}", response_model=dict)
async def update_user(id_user: int, user_data: UserUpdate, request: Request, db: Session = Depends(get_db)):
    current_entity = request.state.current_entity
    updated_user = update_firebase_user(id_user, user_data, db, current_entity)
    return {"id": updated_user.id, "nombre": updated_user.nombre, "email": updated_user.email, "rol": updated_user.rol}

@router.delete("/delete-user/{id_user}", response_model=dict)
async def delete_user(id_user: int, request: Request, db: Session = Depends(get_db)):
    current_entity = request.state.current_entity
    result = delete_firebase_user(id_user, db, current_entity)
    return result
...
@router.put("/update-cuadrilla/{id_cuadrilla}", response_model=dict)
async def update_cuadrilla(id_cuadrilla: int, cuadrilla_data: CuadrillaUpdate, request: Request, db: Session = Depends(get_db)):
    current_entity = request.state.current_entity
    updated_cuadrilla = update_firebase_cuadrilla(id_cuadrilla, cuadrilla_data, db, current_entity)
    return {"id": updated_cuadrilla.id, "nombre": updated_cuadrilla.nombre, "email": updated_cuadrilla.email, "zona": updated_cuadrilla.zona}

@router.delete("/delete-cuadrilla/{id_cuadrilla}", response_model=dict)
async def delete_cuadrilla(id_cuadrilla: int, request: Request, db: Session = Depends(get_db)):
    current_entity = request.state.current_entity
    result = delete_firebase_cuadrilla(id_cuadrilla, db, current_entity)
    return result